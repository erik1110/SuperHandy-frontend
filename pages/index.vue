  ...
  <div>
    ...
    <HomeSecWrapper class="section-2 sp-py-20" bgColor="sp-bg-primary-lighten">
      <div ref="sec2Img" class="sm:sp-basis-1/2 sp-flex-center">
        <img
          class="sp-max-w-[450px] sp-min-w-[300px]"
          src="../assets/images/bg/bg-s2-left.png"
          alt=""
        />
      </div>
      ...
      <div class="sm:sp-basis-1/2 sp-flex-center sp-flex-col">
        <div
          class="introCard sp-mb-4 sp-mx-2 sp-hidden"
          v-for="(cardData, idx) in intro"
          :key="idx"
        >
    ...
    <HomeSecWrapper bgColor="sp-bg-primary">
      <div
        v-for="(cnt, idx) in _countData"
        :key="idx"
        class="sp-basis-full sm:sp-basis-1/2 sp-p-8 sp-max-w-[450px]"
      >
    ...
    <HomeSecWrapper class="s4">
      ...
      <div class="sp-grow sp-py-4 lg:sp-pl-8">
        <HomeCompletedCard
          v-for="item in completedCaseData"
          :key="item._id"
          :card-data="item"
        />
      </div>
      <div class="sp-basis-full sp-mt-8">
        <div class="sp-flex-center sp-mb-20">
          <NuxtLink to="/post-task/-1">
            <v-btn class="text-white" max-width="200" color="v-purple" rounded
              >立即刊登任務
    ...
    <!-- Sec-5-Helpers -->
    <HomeSecWrapper
      bgColor="sp-bg-primary-lighten"
      class="sm:sp-py-32"
      nowrap="true"
    >
      <div
        class="sp-flex-none sp-border-l-[12px] sp-border-primary sp-pl-4 sp-mb-4 sp-w-full sm:sp-w-auto sm:sp-mr-20"
      >
      ...
      >
        <Swiper
          ...
          :navigation="{ nextEl: '.slideNext-btn' }"
          :breakpoints="{
            '320': {
              slidesPerView: 2,
            },
      ...
      <div class="sp-flex-initial sp-basis-full sm:sp-hidden">
        <HomeHelperCard
          @click="openUserDialog(h)"
          v-for="(h, idx) in helpersSm"
          :key="idx"
          v-bind="h"
        />
      </div>
    ...
    <HomeSecWrapper class="s6">
      ...
      <div class="s6_right sm:sp-justify-between">
        <HomeCommentCard
          class="hover:sp--translate-y-2 sp-transition-all"
          v-bind="c"
