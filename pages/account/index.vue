    ...
    <div class="sp-card-wrapper sp-bg-white sp-p-6">
      <div class="sm:sp-flex sm:sp-items-center sm:sp-space-x-4 sp-mb-6">
        ...
        <div class="sp-mt-2 sm:sp-mt-0">
          ...
          <p class="sp-text-gray-placeholder" v-show="userData.email">
            帳號 {{ userData.email }}
          </p>
        </div>
      </div>
      <div class="sp-mb-4">
        <!-- 六個數字 -->
        <AccountPerformance
          :hosterData="performanceHosterData"
          :helperData="performanceHelperData"
        ></AccountPerformance>
      </div>
    </div>
    <div class="sp-card-wrapper sp-bg-white sp-p-6">
      ...
      <v-form @submit.prevent="submit" ref="accountForm" validate-on="input">
        <div class="mt-4 md:sp-w-1/2">
          ...
          <v-text-field
            ...
            :rules="accountFormRules.nickname.rule"
            id="nickname"
        ...
        <div class="mt-4">
          ...
          <v-textarea
            v-model="userData.posterIntro"
            :rules="accountFormRules.intro.rule"
            :counter="accountFormRules.intro.counter"
            :hint="accountFormRules.intro.hint"
          />
        </div>
        <div class="mt-4">
          <label class="label" for="nickname">幫手介紹</label>
          <v-textarea
            v-model="userData.helperIntro"
            :rules="accountFormRules.intro.rule"
            :counter="accountFormRules.intro.counter"
            :hint="accountFormRules.intro.hint"
          />
        ...
        <div class="mt-4">
          <label
            class="label text-grey-darken-2 d-flex align-center"
            for="helperSkills"
            >幫手專長
            <v-chip class="ma-1" color="red" text-color="white" size="x-small">
              最多可以選三項
          ...
          <v-select
            v-model="userData.helperSkills"
            :rules="accountFormRules.helperSkills.rule"
        ...
        <div class="mt-8">
          <v-btn
            type="submit"
            color="v-purple"
            class="sp-px-4 sp-w-full md:sp-mb-0 md:sp-w-auto"
...
const { formRules, validateFormResult, ruleAddress, ruleRequired } =
  useFormUtil();
...
provide("hintMsgs", {
  counter: {
    locationCity: "",
    locationDist: "",
    locationAddress: accountFormRules.address.counter,
  },
  hint: {
    locationCity: "",
    locationDist: "",
    locationAddress: accountFormRules.address.hint,
  },
});
provide("currentRules", {
  locationCity: [ruleRequired],
  locationDist: [ruleRequired],
  locationAddress: [ruleRequired, ruleAddress],
});
provide("currentFieldDisabled", {
  locationCity: false,
  locationDist: false,
...
const init = () => {
  ...
  const promiseArr = [
    ...
    excuteAsyncFunc(
      ...
      getCategories,
      null,
      (response) => (taskCategories.value = response.data)
    ),
