          ...
          >
            <v-card-text>
              ...
              <div class="sp-min-h-[20px] sp-mb-2">
                ...
                <div
                  class="sp-float-right sp-px-2 sp-py-1 sp-border-2 sp-border-solid sp-border-gray-500 sp-rounded-sm"
                >
                  {{ item.role }}
                </div>
              </div>
              ...
              >
                ...
                <template v-else>
                  <template v-if="item.money.superCoin > 0">+</template
                  >{{ item.money.superCoin + item.money.helperCoin }}({{
                    item.money.superCoin
                  }}
                  超人幣<template v-if="item.money.helperCoin != 0"
                    >，{{ item.money.helperCoin }} 幫手幣</template
                  >)：{{ item.desc.toString() }}
                </template>
              </div>
              <!-- history create date -->
